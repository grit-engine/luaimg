...
local C = vector3(tonumber(args[1]), tonumber(args[2]), tonumber(args[3])) / 255
local input_name = args[4]
local output_name = args[5]

-- avoid divide by zero errors
C = vector3(clamp(C.x, 0.00001, 0.99999), clamp(C.y, 0.00001, 0.99999), clamp(C.z, 0.00001, 0.99999))

open(input_name):map(4, function(I)
    if I == vector3(0,0,0) then
        return vector4(I,1)
    end
    --[[
        I = a * s + (1-a) * C
